# Telangana Registration Portal Automation
...
3. **Configure Environment:**
   ...
   cp .env.example .env
   ```
   *(Update the `.env` file with your specific paths if necessary).*
...
npm run collect
```
*Expected Output:* The script launches a browser. The user manually logs in, solves the initial CAPTCHA, and presses ENTER in the terminal. The session is serialized to `session.json`.

### Phase 2: Automation Execution
Once the session is serialized, the core automation can be executed.
```bash
npm run dev
```
*Expected Output:* The script injects the session, navigates across domains seamlessly, fills the search criteria, pauses for the secondary CAPTCHA, and downloads the document to the `downloads/` directory.
